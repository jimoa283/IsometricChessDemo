    ...
    public class SaveCSV
    {
        ...
        public void Save()
        {
            ...
            foreach (DialogueContainerSO dialogueContainer in dialogueContainers)
            {
                ...
                {
                    /*foreach(DialogueData_Text textData in nodeData.DialogueData_Text)
                    {
                    ...
                    texts.Add(dialogueContainer.name);
                    texts.Add(nodeData.NodeGuid);
                    texts.Add(nodeData.DialogueData_Text.GuidID.Value);

                    foreach (LanguageType languageType in (LanguageType[])Enum.GetValues(typeof(LanguageType)))
                    {
                        string tmp = nodeData.DialogueData_Text.Text.Find(language => language.languageType == languageType).LanguageGenericType.Replace("\"", "\"\"");
                        texts.Add($"\"{tmp}\"");
                    }

...
public class MagicDamageCalculation : IImpactEffect
{
    public void DoEffect(BattleObj battleObj)
    {
        /*foreach (var target in battleObj.targetPieces)
        {
            ...
            {
                int damage = battleObj.baseActionData.Power + battleObj.owner.pieceStatus.Magic - target.pieceStatus.MagicDefense;
                if (damage < 0)
                    damage = 0;
